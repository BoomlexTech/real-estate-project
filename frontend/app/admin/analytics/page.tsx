...
import {
  ...
  deleteAdminBrochureLead,
  BrochureAnalyticsItem,
  BrochureLead,
} from '@/lib/adminApi';
...
export default function AdminAnalyticsPage() {
  ...
  const [items, setItems] = useState<BrochureAnalyticsItem[]>([]);
  ...
  const handleDelete = async (leadId: string, propertyId: string) => {
    setDeleting(leadId);
    try {
      await deleteAdminBrochureLead(leadId);
      setItems((prev) =>
        prev.map((item) =>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm" style={{ color: palette.textSecondary }}>
          Leads captured when visitors download property brochures · auto-expire after 30 days
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { icon: <FileDown size={18} />, label: 'Total Properties', value: totalProps, accent: false },
        ].map((s) => (
          <div
            key={s.label}
      ...
      {!loading && !error && totalLeads === 0 && (
        <div className="text-center py-20 rounded-2xl" style={{ background: palette.cardBg, border: `1px dashed ${palette.border}` }}>
          ...
          <p className="text-xs" style={{ color: palette.textSecondary }}>Leads will appear here when visitors download property brochures.</p>
        </div>
      )}

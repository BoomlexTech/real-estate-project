...
type FilterStatus = 'all' | PropertyInquiryStatus;

const STATUS_OPTIONS: PropertyInquiryStatus[] = ['new', 'contacted', 'closed'];

const statusStyle: Record<PropertyInquiryStatus, { bg: string; color: string }> = {
...
export default function AdminPropertyInquiriesPage() {
  ...
  const fetchInquiries = async (p: number, s: FilterStatus) => {
    ...
    setError('');
    try {
      const res = await getPropertyInquiries(p, s === 'all' ? undefined : s);
  ...
  const handleStatusChange = async (id: string, status: PropertyInquiryStatus) => {
    setUpdatingId(id);
    try {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: palette.textPrimary }}>Property Inquiries</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: `1px solid ${palette.border}` }}>
                  {['Name', 'Email', 'Phone', 'Property', 'Agent', 'Message', 'Status', 'Date', 'Update Status'].map((h) => (
                ...
                {inquiries.map((inq) => {
                  ...
                  return (
                    <tr key={inq._id} style={{ borderBottom: `1px solid ${palette.border}` }}>
                      <td className="px-4 py-3 font-medium whitespace-nowrap" style={{ color: palette.textPrimary }}>{inq.name}</td>
                      <td className="px-4 py-3" style={{ color: palette.textSecondary }}>{inq.email}</td>
                      <td className="px-4 py-3 whitespace-nowrap" style={{ color: palette.textSecondary }}>{inq.phone || '—'}</td>
                      <td className="px-4 py-3 whitespace-nowrap" style={{ color: palette.textPrimary }}>{inq.property?.title || '—'}</td>
                      <td className="px-4 py-3 whitespace-nowrap" style={{ color: palette.textPrimary }}>{inq.agent?.name || '—'}</td>
                      <td className="px-4 py-3 max-w-xs" style={{ color: palette.textSecondary }}>
                        <span title={inq.message}>{inq.message.length > 60 ? inq.message.slice(0, 60) + '…' : inq.message}</span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          onChange={(e) => handleStatusChange(inq._id, e.target.value as PropertyInquiryStatus)}
